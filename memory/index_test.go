...
func TestIndexConversationV2WithExtraction(t *testing.T) {
	dir := t.TempDir()
	mdb, err := Open(filepath.Join(dir, "memory.db"))
	if err != nil {
		t.Fatal(err)
	...
	err = mdb.IndexConversationV2(ctx, "conv-v2-1", "Auth Chat", messages, nil, mockSvc)
	if err != nil {
		t.Fatalf("IndexConversationV2: %v", err)
	}
	...
	err = mdb.IndexConversationV2(ctx, "conv-v2-1", "Auth Chat", messages, nil, mockSvc)
	if err != nil {
		t.Fatalf("re-index IndexConversationV2: %v", err)
	}
...
func TestIndexConversationV2FallbackWithoutLLM(t *testing.T) {
	dir := t.TempDir()
	mdb, err := Open(filepath.Join(dir, "memory.db"))
	if err != nil {
		t.Fatal(err)
	...
	err = mdb.IndexConversationV2(ctx, "conv-v2-fallback", "K8s Scheduling", messages, nil, nil)
	if err != nil {
		t.Fatalf("IndexConversationV2 fallback: %v", err)
	}
